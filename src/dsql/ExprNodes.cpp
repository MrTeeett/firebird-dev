...
static RegisterNode<LTrimNode> regLTrimNode({blr_ltrim});

LTrimNode::LTrimNode(MemoryPool& pool, UCHAR aWhere, ValueExprNode* aValue, ValueExprNode* aTrimChars)
    : TypedNode<ValueExprNode, ExprNode::TYPE_LTRIM>(pool),
    where(aWhere),
    value(aValue),
    trimChars(aTrimChars)
{
...
DmlNode* LTrimNode::parse(thread_db* tdbb, MemoryPool& pool, CompilerScratch* csb, const UCHAR /*blrOp*/)
{
    UCHAR where = csb->csb_blr_reader.getByte();
    UCHAR what = csb->csb_blr_reader.getByte();

    LTrimNode* node = FB_NEW_POOL(pool) LTrimNode(pool, where);

    if (what == blr_ltrim_characters)
        node->trimChars = PAR_parse_value(tdbb, csb);
...
string LTrimNode::internalPrint(NodePrinter& printer) const
{
    ValueExprNode::internalPrint(printer);
...
ValueExprNode* LTrimNode::dsqlPass(DsqlCompilerScratch* dsqlScratch)
{
    LTrimNode* node = FB_NEW_POOL(dsqlScratch->getPool()) LTrimNode(dsqlScratch->getPool(), where,
                                                                  doDsqlPass(dsqlScratch, value), doDsqlPass(dsqlScratch, trimChars));
...
void LTrimNode::setParameterName(dsql_par* parameter) const
{
    parameter->par_name = parameter->par_alias = "LTRIM";
}

bool LTrimNode::setParameterType(DsqlCompilerScratch* dsqlScratch,
                                std::function<void (dsc*)> makeDesc, bool forceVarChar)
{
    return PASS1_set_parameter_type(dsqlScratch, value, makeDesc, forceVarChar) |
           PASS1_set_parameter_type(dsqlScratch, trimChars, makeDesc, forceVarChar);
...
void LTrimNode::genBlr(DsqlCompilerScratch* dsqlScratch)
{
    dsqlScratch->appendUChar(blr_ltrim);
    dsqlScratch->appendUChar(where);

    if (trimChars)
    {
        dsqlScratch->appendUChar(blr_ltrim_characters);
        GEN_expr(dsqlScratch, trimChars);
    }
    else
        dsqlScratch->appendUChar(blr_ltrim_spaces);

    GEN_expr(dsqlScratch, value);
}

void LTrimNode::make(DsqlCompilerScratch* dsqlScratch, dsc* desc)
{
    dsc desc1, desc2;
...
void LTrimNode::getDesc(thread_db* tdbb, CompilerScratch* csb, dsc* desc)
{
    value->getDesc(tdbb, csb, desc);
...
ValueExprNode* LTrimNode::copy(thread_db* tdbb, NodeCopier& copier) const
{
    LTrimNode* node = FB_NEW_POOL(*tdbb->getDefaultPool()) LTrimNode(*tdbb->getDefaultPool(), where);
    node->value = copier.copy(tdbb, value);
    if (trimChars)
        node->trimChars = copier.copy(tdbb, trimChars);
    return node;
...
bool LTrimNode::dsqlMatch(DsqlCompilerScratch* dsqlScratch, const ExprNode* other, bool ignoreMapCast) const
{
    if (!ExprNode::dsqlMatch(dsqlScratch, other, ignoreMapCast))
        return false;

    const LTrimNode* o = nodeAs<LTrimNode>(other);
    fb_assert(o);

    return where == o->where;
}

bool LTrimNode::sameAs(const ExprNode* other, bool ignoreStreams) const
{
    if (!ExprNode::sameAs(other, ignoreStreams))
        return false;

    const LTrimNode* const otherNode = nodeAs<LTrimNode>(other);
    fb_assert(otherNode);

    return where == otherNode->where;
}

ValueExprNode* LTrimNode::pass2(thread_db* tdbb, CompilerScratch* csb)
{
    ValueExprNode::pass2(tdbb, csb);
...
// Perform trim function = TRIM([where what FROM] string).
dsc* LTrimNode::execute(thread_db* tdbb, Request* request) const
{
    impure_value* impure = request->getImpure<impure_value>(impureOffset);
    request->req_flags &= ~req_null;
    ...
    if (charactersCanonicalLen)
    {
        if (where == blr_ltrim_both || where == blr_ltrim_leading)
        {
            // CVC: Prevent surprises with offsetLead < valueCanonicalLen; it may fail.
            for (; offsetLead + charactersCanonicalLen <= valueCanonicalLen;
                 offsetLead += charactersCanonicalLen)
